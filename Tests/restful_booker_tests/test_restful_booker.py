...
def test_get_booking(base_url, create_booking_id):
...
def test_get_booking_negative(base_url, booking_id):
    response = requests.get(f"{base_url}/booking/{booking_id}")
    assert response.status_code == 404, "booking ID with invalid format exist"
    assert "application/json" not in response.headers.get("Content-Type", ""), "booking ID with invalid format has a Content-type application/json"
    if response.headers.get("Content-Type", "").startswith("application/json"):
        body = response.json()
        assert "firstname" not in body, "First name data is present in booking ID with invalid format"

def test_update_booking(base_url, create_booking_id, auth_cookie_token):
    booking_payload = {
    "firstname": "Updated",
    "lastname": "User",
    "totalprice": 999,
    ...
    response = requests.put(f"{base_url}/booking/{create_booking_id}", cookies=auth_cookie_token, json=booking_payload)
    put_body = response.json()

    assert booking_payload["firstname"] in put_body['firstname'], f"First Name - {booking_payload["firstname"]} was not updated in put_body"
    assert booking_payload["lastname"] in put_body['lastname'], f"lastname Name - {booking_payload["lastname"]} was not updated in put_body"
    ...
    assert booking_payload["firstname"] in get_body['firstname'], f"First Name - {booking_payload["firstname"]} was not updated after put request"
    assert booking_payload["lastname"] in get_body['lastname'], f"lastname Name - {booking_payload["lastname"]} was not updated after put request"

    assert response.status_code == 200, f"Access forbidden, verify auth cookies"

def test_update_booking_without_valid_token(base_url, create_booking_id):
    booking_payload = {
        "firstname": "Updated",
        "lastname": "User",
        "totalprice": 999,
...
def test_patch_booking(base_url, create_booking_id, auth_cookie_token):
    booking_payload = {
        "firstname": "Mikie",
        "lastname": "Howard"}
    response = requests.patch(f"{base_url}/booking/{create_booking_id}", cookies=auth_cookie_token, json=booking_payload)
    assert response.status_code == 200, "Patch request is failed"
    patch_body = response.json()

    assert booking_payload["firstname"] in patch_body['firstname'], f"First Name - {booking_payload["firstname"]} was not updated in patch_body"
    assert booking_payload["lastname"] in patch_body['lastname'], f"lastname Name - {booking_payload["lastname"]} was not updated in patch_body"

    get_response = requests.get(f"{base_url}/booking/{create_booking_id}")
    get_body = get_response.json()

    assert booking_payload["firstname"] in get_body['firstname'], f"First Name - {booking_payload["firstname"]} was not updated after patch request"
    assert booking_payload["lastname"] in get_body['lastname'], f"lastname Name - {booking_payload["lastname"]} was not updated after patch request"

def test_delete_booking(base_url, auth_cookie_token, create_booking_id):
...
def test_delete_booking_without_token_negative(base_url, create_booking_id):
